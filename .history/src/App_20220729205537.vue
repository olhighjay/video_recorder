    ...
    <div class="record-page">
      <div class="view-frame-wrapper">
        ...
        <div class="view-frame">
          ...
          <h3>Recording..</h3> &nbsp;&nbsp; <def-loader></def-loader>
          </div>
          ...
          <!-- <div v-if="true" style="height:100%"> -->
            ...
            <video v-show="recorded " id="my-recorded-video" controls class="recorded-video bg-black w-full h-auto"></video>
          </div>
        </div>
        <div class="text-center start-recording-wrapper">
          ...
          <button v-if="recordVideo && !currentStream" @click="startCamVideo" class="start-recording">Record Camera</button>
          <button v-if="recordVideo && currentStream === 'camR'" @click="stopAndSaveRecordingCamVideo(mediaVideoRec)" class="start-recording">Stop recording</button>

          <button v-if="recordScreen && !currentStream" @click="startScreenRecording" class="start-recording">Record Screen</button>
          <button v-if="recordScreen && currentStream === 'scnR'" @click="stopScreenRecording" class="start-recording">Stop recording</button>
...
let videoTwo = document.getElementById('my-recorded-video');
...
export default {
  ...
  methods: {
    ...
    async handleStop (e) {
      ...
      this.recordedVideo.src = this.downloadLink = URL.createObjectURL(blob);
      this.recordedVideo.load();
      this.recordedVideo.onloadeddata = function() {
  ...
  async created() {
    // window.removeEventListener('onload',this.recordVideo)
    this.recordedVideo = await document.getElementById('my-recorded-video');
